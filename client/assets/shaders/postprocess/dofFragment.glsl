...
float width = u_size.x; //texture width
float height = u_size.y; //texture height

vec2 texel = vec2(1.0/width,1.0/height);

const float focalDepth = 0.1;  //external focal point value, but you may use autofocus option below
...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = texture(u_textureDepth, coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
vec3 color(vec2 coords,float blur) //processing the sample
{
	...
	col.r = texture(u_texture,coords + vec2(0.0,1.0)*texel*fringe*blur).r;
	col.g = texture(u_texture,coords + vec2(-0.866,-0.5)*texel*fringe*blur).g;
	col.b = texture(u_texture,coords + vec2(0.866,-0.5)*texel*fringe*blur).b;
...
void main() 
{
	...
	float depth = texture(u_textureDepth,textureCoords.xy).x;
	float blur = 0.0;
	
	if (depthblur)
	{
		depth = bdepth(textureCoords.xy);
	}
	...
	if (autofocus)
	{
		float fDepth = texture(u_textureDepth,focus).x;
		blur = clamp((abs(depth - fDepth)/range)*100.0,-maxblur,maxblur);
	}
	
	vec2 noise = rand(textureCoords.xy)*namount*blur;
	
	float w = (1.0/width)*blur+noise.x;
	float h = (1.0/height)*blur+noise.y;
	
	vec3 col = texture(u_texture, textureCoords.xy).rgb;
	float s = 1.0;
	...
	{   
		...
		for (int j = 0 ; j < ringsamples ; j += 1)   
		{
			...
			col += color(textureCoords.xy + vec2(pw*w,ph*h),blur)*mix(1.0,(float(i))/(float(rings)),bias)*p;  
