...
uses
  Windows, CoreX, NvTriStrip, xmt, a1_java, SysUtils;
...
type
  ...
  TNodeMesh = object
    ...
    Mesh   : TMesh;
    JName  : TStrArray;
    TexCoordCollada : Boolean; // инвертировать Y координату?
  private
  ...
  public
    ...
    procedure SaveA1(const FileName: string);
  end;
{$ENDREGION}
...
procedure TNodeMesh.SaveA1(const FileName: string);
const
  Mode : TMeshMode = mmTriList;
  AttribSize : array [TMaterialAttrib] of LongInt = (12, 4, 4, 8, 8, 4, 4);
var
...
//  v2s : TVec2s;
  ...
  ffname : string;
begin
  Writeln('save a1 : ' , FileName);

  ffname := FileName + '.ams';
  ss := TStream.Init('c:\001.ams', True);
  if ss <> nil then
  begin
//    Stream.Write(Params, SizeOf(Params));

//    Stream.Write(BBox, SizeOf(BBox));
    TJavaStream.WriteVec3f(ss, bbox.Min);
    TJavaStream.WriteVec3f(ss, bbox.Max);

    ss.Write(Mode, SizeOf(Mode));

  // attrib ---------------------------------------
    Count := Ord(High(ma)) - Ord(Low(ma)) + 1;
    TJavaStream.WriteInt(ss, Count);

    for ma := Low(ma) to High(ma) do
        if ma in Attrib then
        begin
          bb := 1;
          ss.Write(bb, SizeOf(bb));
        end else begin
          bb := 0;
          ss.Write(bb, SizeOf(bb));
        end;
//    ss.Write(Attrib, SizeOf(Attrib));

  // indices --------------------------------------
    Stride := SizeOf(Index[0]);
    Count  := Length(Index);
//    ss.Write(Count, SizeOf(Count));
    TJavaStream.WriteInt(ss, Count);
    if Count > 0 then
    begin
      Stride := SizeOf(Index[0]);
//      ss.Write(Stride, SizeOf(Stride));
      TJavaStream.WriteInt(ss, Stride);
      for i := 0 to Count - 1 do
        ss.Write(Index[i], Stride);
    end;
...
//                  v2s.x := Clamp(LongInt(Round(v2f.x * 1024)), Low(SmallInt), High(SmallInt));
//                  v2s.y := Clamp(LongInt(Round(v2f.y * 1024)), Low(SmallInt), High(SmallInt));
//                  ss.Write(v2s, SizeOf(v2s));
                  ss.Write(v2f, SizeOf(v2f))
                end;
              maColor :
                begin
                  with Color do
...
//    ss.Write(Count, SizeOf(Count));
    TJavaStream.WriteInt(ss, Count);
    for i := 0 to Count - 1 do
      ss.WriteAnsi(AnsiString(JName[i]));
    ss.Free;

    DeleteFile(ffname);
    CopyFile('c:\001.ams', PChar(ffname), false);
  end else
    Error('Can''t save "' + FileName + '"');
end;
...
// save to file
  ss := TStream.Init(OUT_DIR + Name + '.ask', True);
  if ss <> nil then
  begin
    w := Length(Joint);
    ss.Write(w, SizeOf(w));
...
//      ss.Write(Joint[i].Parent, SizeOf(Joint[i].Parent));
      TJavaStream.WriteDualQuat(ss, Joint[i].Bind);
//      ss.Write(Joint[i].Bind, SizeOf(Joint[i].Bind));
      TJavaStream.WriteDualQuat(ss, Joint[i].Frame);
//      ss.Write(Joint[i].Frame, SizeOf(Joint[i].Frame));
    end;
    ss.Free;
  end;
end;
...
//    rewrite(frotpos);
    ...
    Stream := TStream.Init(OUT_DIR + Name + '.aan', True);
    if Stream <> nil then
    begin
      i := Length(Joint);
...
begin
  with Nodes[NodeIdx] do
    ...
    begin
      ...
      Nodes[NodeIdx].Mesh.TexCoordCollada := true;
...
// Save skeleton
  ...
  if Skinned then begin
    Writeln('save skeleton: ', DeleteExt(ExtractFileName(FileName)) + ' ...');
...
//      Nodes[i].Material.Save(OUT_DIR + Nodes[i].Material.Name);
//      Nodes[i].Material.Material := TMaterial.Load(Nodes[i].Material.Name);



      Nodes[i].Mesh.Name := Nodes[i].MeshURL + '_' + Nodes[i].MatSymbol;
      Writeln('save mesh: ', Nodes[i].Mesh.Name + ' ...');
      Nodes[i].Mesh.SaveA1(OUT_DIR + Nodes[i].Mesh.Name);
