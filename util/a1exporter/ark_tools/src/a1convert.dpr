...
uses
  ...
  a1_material,
  ShellAPI,
  a1_max_loader in 'a1_max_loader.pas',
  a1_blender_loader in 'a1_blender_loader.pas';
...
begin
  ...
  begin
    fname_work := ParamStr(1);
    fname_ext := ExtractFileExt(fname_work);
  ...
  //  ReportMemoryLeaksOnShutdown := True;
    ...
    if fname_ext = '.a1blend' then
      ConvertBlender(fname_work);

    if fname_ext = '.dae' then
      Convert(fname_work);

    if fname_ext = '.xml' then
      ConvertMaterial(fname_work);

//    ShellExecute( 0, 'open', 'copy.bat', '', '.', 0 );

  //  CoreX.Start(@OnInit, @OnFree, @OnRender);
  end else begin
    Writeln('need filename in params to convert');
  end;
end.
